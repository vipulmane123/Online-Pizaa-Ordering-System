...
function ReviewsList() {
    ...
    const getReviews=async()=>{
        try {
            debugger;
        await axios.get(`http://localhost:8080/pizzadelivery/productreviews/${id}`).then((res)=>{
            if(res!=null){
                setReviews(res.data)
            } else {
  ...
  return (
    ...
    <div className='container' style={{"marginTop":"70px","marginBottom":"40px"}}>
        ...
        <button className='btn btn-primary' onClick={()=>{history.push(`/addreview/${id}`)}} style={{margin:"10px"}}>POST REVIEW</button>
            <div className="row">
